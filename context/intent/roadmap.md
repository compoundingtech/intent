...
- **Planning-tool integration:** Intent should eventually integrate with whichever
  tool owns durable plan intent, so long-running agent work can reference Intent
  artifacts, propose Intent changes, attach Intent evidence, and route durable
  ...
  - Promotion target: requirements, spec, or a dedicated integration child Intent.

## Structured Intent Layer

- **Structured Intent representation:** Intent should eventually move beyond a brittle
  ...
  - Trigger: evidence shows a structured representation improves Intent
  ...
  - Promotion target: requirements, spec, enforcement, or a dedicated structured
    Intent child node.
- **Typed graph v0:** Before a full typed-source model, Intent should define a
  ...
  - Promotion target: enforcement spec or a dedicated structured Intent child node.
