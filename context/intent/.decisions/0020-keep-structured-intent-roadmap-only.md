# Keep structured Intent roadmap-only for now

Status: accepted

## Context

Intent currently uses Markdown files plus deterministic conventions. The roadmap
...
The current implementation path is still centered on Markdown-authored Intent
...
from `intent` enforcement and isolated evaluations decide the next step.
...
| Move to typed Intent source | Strongest long-term tooling model, but highest migration cost and premature for the current checker maturity. |

## Decision

Keep structured Intent representation roadmap-only for now. Markdown Intent artifacts
...
Revisit this decision when `intent` enforcement repeatedly needs facts that
