...
invites stale completed work and mixed task lists; `.delta/` is the only VRS
delta shape.
...
update implementation | update VRS | decide

## Resolution Signal
```

`Divergence` states the mismatch in one or two sentences. `VRS` links to the
...
normative VRS is incomplete, contradictory, or false.

## Lifecycle

Agents must check affected deltas whenever they edit referenced VRS artifacts or
...
divergence. If the divergence was intentional, first update the relevant VRS
