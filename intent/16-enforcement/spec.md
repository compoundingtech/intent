# VRS Enforcement — Spec

This document specifies VRS enforcement for real repository VRS trees. It builds
...
Enforcement checks whether VRS artifacts obey the meta-VRS contract. It does not
...
| --- | --- | --- |
| Evaluation | Temporary scenario outside tracked files | Evidence about whether VRS/skills work end to end |
| Enforcement | Real repository VRS artifacts | Findings about hygiene, validity, and review risk |
...
| Semantic review | Vision purity, requirement testability, spec completeness, decision substance, open-question freshness, review-smell judgment | Advisory until a VRS or project policy makes it blocking |
| Eval feedback | Reproduces false positives, missing checks, and ambiguous diagnostics in isolated scenarios | Evidence for VRS updates |
...
- VRS node detection and allowed lazy companion directories.
- Numeric child directory names (`NN-slug`) where child VRS nodes are used.
...
This is not a full typed VRS graph. The first checker should avoid semantic
...
initial decision-shape experiment found the meta-VRS decisions compatible with
...
`context/vrs/.decisions/`. Other `context/**/.decisions/` records may be scanned
...
- [review-prompt.md](./review-prompt.md) defines the default `axe vrs review`
  prompt and routes semantic judgment to the owning VRS contracts.
- [review-result.schema.json](./review-result.schema.json) defines the
  `axe.vrs.review.v1` result shape.

`axe vrs review` consumes these assets through the Coding Agent Invocation
Contract. Callers do not provide arbitrary prompts for the standard review
mode; prompt and schema changes are VRS changes and should be evaluated through
fixtures before becoming the baked version.

Real-provider execution policy is owned by the
[VRS command spec](../../context/cli/spec.md). Enforcement
...
```json
{
  ...
  "artifact": "context/vrs/requirements.md",
  "owner": "02-requirements",
  "rule": "VRS.ENF.<rule-id>",
  "evidence": "Requirement IDs skip VRS-R12.",
...
checks; those stay in the relevant meta-VRS nodes.
...
and does not silently rewrite VRS.

`grill-vrs` may call enforcement during a VRS session, but it remains the
interview/update procedure. Enforcement supplies findings; the owning VRS
...
specific VRS or project policy after evidence shows that the review mode is
...
questions owned by the affected VRS artifact.
