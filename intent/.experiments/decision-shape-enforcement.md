...
Can decision-record shape checks become a useful deterministic VRS enforcement
rule without pretending to judge semantic decision quality?

## Hypothesis

Decision-record shape checks can become a useful deterministic VRS enforcement
...
- fixture/prototype direction: design good and bad decision records and test
  ...
  records;
- corpus dry run: scan existing decision records for the meta-VRS minimum
  shape;
...
The meta-VRS corpus is compatible with the proposed mechanical minimum: the
current `context/vrs/.decisions/000*.md` records use the required sections and
options-table shape.
...
valuable for the meta-VRS corpus, but broad blocking promotion should wait for
