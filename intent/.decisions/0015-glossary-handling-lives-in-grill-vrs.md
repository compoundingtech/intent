# Glossary handling lives in Grill VRS
...
should be part of `grill-vrs` rather than a separate glossary skill by default.
...
| Keep glossary rules only in meta-VRS | Normative, but does not say who applies them during interviews. |
| Make `grill-vrs` apply the glossary contract | Simple flow and one less skill, while preserving the option to split later. |

## Decision

`grill-vrs` handles glossary work by applying `context/vrs/04-glossary/`.
