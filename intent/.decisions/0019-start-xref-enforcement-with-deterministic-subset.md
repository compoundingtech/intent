...
After deciding that VRS identifiers are commit-scoped consistency handles, the
...
links are ambiguous in hierarchical VRS, and symbolic or namespaced IDs make
refactors easier when a resolver can check them.

## Evidence and Argument

The current VRS representation is still Markdown-authored. A full typed graph is
...
| Option | Tradeoffs |
| --- | --- |
| Wait for typed VRS graph | Cleaner architecture later, but leaves current brittle links unchecked. |
