...
The Notion notes proposed a separate `datamodel.md` and listed other good VRS
...
common VRS subsystem rather than a generic companion file.
...
would create a special case and compete with the hierarchical VRS model. Common
...
| Treat data model and peers as common child subsystems | Scales through normal VRS composition and keeps companion files for lifecycle artifacts. |
...
model becomes a child VRS node with its own `requirements.md` and `spec.md`, not
`datamodel.md`.
