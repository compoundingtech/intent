# Intent Ontology — Requirements

## Context

- This child node defines the contract for `ontology.md` files in Intent nodes.
- It refines [INTENT-R01](../requirements.md) and [INTENT-R02](../requirements.md).
...
- **INTENT.ONT-R01 Canonical terms:** An ontology must choose one canonical term
  when multiple names compete.
- **INTENT.ONT-R02 Tight definitions:** Definitions must describe what a thing is,
  not how it is implemented.
- **INTENT.ONT-R03 Avoid aliases:** Discouraged aliases must be listed when they
  are likely to appear in code, docs, or discussion.

### May Capture Structure

- **INTENT.ONT-R06 Lazy structure:** An ontology may record how terms relate —
...
- **INTENT.ONT-R07 Legible membership:** When a term is a follower of an anchor,
  its name should carry the anchor's leitwort so membership is legible without
  a lookup.
- **INTENT.ONT-R08 Declared inheritance:** A child ontology must name the parent
...
- **INTENT.ONT-R04 No decisions:** An ontology must not carry rationale or decision
  history.
- **INTENT.ONT-R05 No implementation detail:** An ontology must not become an API,
